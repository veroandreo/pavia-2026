...
This optional part of the workshop shows the use of R to model *Aedes albopictus* distribution in Northern Italy. 
For that, we need to connect to GRASS via the `rgrass` package in order to read occurrence data and predictors. The *rgrass* package is developed by @rgrass and can be found at: <https://github.com/rsbivand/rgrass/>. See the vignette with further explanations and examples at: <https://rsbivand.github.io/rgrass/>.

# [**rgrass**](https://cran.r-project.org/web/packages/rgrass/index.html)

- `initGRASS()`: starts a GRASS session from R
- `execGRASS()`: executes GRASS commands 
- `gmeta()`: shows GRASS location metadata
- `read_VECT()` and `read_RAST()`: read vector and raster maps from GRASS into R *terra* objects.
- `write_VECT()` and `write_RAST()`: write R *terra* objects into the GRASS database
...
::: {.callout-note}
See the [**Get started with GRASS & R**](https://grass-tutorials.osgeo.org/content/tutorials/get_started/fast_track_grass_and_R.html) tutorial for a 101 guide on interfacing GRASS and R.
:::

# SDM workflow

Here we'll be covering the middle and right side of the SDM workflow, modeling and predictions.

![](../assets/img/lecture_1/workflow_sdm_other.png)
...
Let's launch GRASS in our project and mapset, from R
...
grassdata <- "/content/drive/MyDrive/pavia_2026"
# path to location
project <- "eu_laea"
# path to mapset
...
mapview(raster::raster(predictors_r[['bioclim_bio01']])) + presence
```
...
There are many other relevant packages we could use: 
